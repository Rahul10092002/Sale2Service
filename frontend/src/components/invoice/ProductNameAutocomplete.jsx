...
export default function ProductNameAutocomplete({
  ...
  return (
    <div className="relative">
      ...
      {error && (
        <p className="mt-1 text-xs sm:text-[11px] text-danger flex items-center">
      ...
      {showDropdown && (
        <ul
          className="absolute z-50 left-0 right-0 mt-1 bg-white dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-lg shadow-dropdown max-h-64 sm:max-h-60 overflow-auto divide-y divide-gray-100 dark:divide-dark-border"
          onMouseDown={(e) => e.preventDefault()}
        >
          {isLoading && (
            <li className="px-3 py-2.5 text-xs sm:text-[11px] text-gray-500 italic flex items-center gap-2">
          ...
          {!isLoading && suggestions.length === 0 && (
            <li className="px-3 py-3 text-xs sm:text-[11px] text-gray-400">No matching products found</li>
          )}
          ...
          {suggestions.map((s, i) => (
            <li
              ...
              className={
                "px-3 py-2.5 sm:py-1.5 cursor-pointer flex items-center justify-between gap-2 min-h-[44px] sm:min-h-0 text-sm sm:text-xs transition-colors " +
                (i === highlightedIndex
                  ? "bg-indigo-50 text-indigo-900 dark:bg-indigo-900/30 dark:text-indigo-300"
                  : "hover:bg-gray-50 active:bg-gray-100 dark:hover:bg-dark-hover dark:active:bg-dark-border text-gray-700 dark:text-slate-200")
              }
            >
              <div className="min-w-0 flex-1">
                <p className="font-medium truncate text-sm sm:text-xs text-gray-900 dark:text-slate-100">
                  {highlight(s.product_name, value)}
                </p>

                {(s.company || s.product_category) && (
                  <p className="text-xs sm:text-[10px] text-gray-500 truncate dark:text-slate-400 mt-0.5">
              ...
              <div className="shrink-0 flex flex-col items-end gap-1">
                {s.selling_price > 0 && (
                  <span className="text-xs sm:text-[10px] font-semibold text-ink-secondary dark:text-slate-300">
                ...
                <span
                  className={
                    "text-[10px] sm:text-[9px] px-1.5 py-0.5 rounded-full font-medium " +
                    (s.source === "history"
